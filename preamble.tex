\documentclass{book}[a5paper]
\usepackage[a5paper, paperwidth=6in, paperheight=9in]{geometry}
\usepackage[utf8]{inputenc}
\usepackage{amsmath}
\usepackage{amsthm}
\usepackage{amssymb}
\usepackage[font=small,labelfont=bf]{caption}
\usepackage{wrapfig}
\usepackage{enumitem}
\usepackage{import}
\usepackage{physics}
\usepackage{siunitx}
\usepackage{titlesec}
\usepackage{enumerate}
\usepackage{mathtools}

% \diff
\usepackage[thinc]{esdiff}
\newcommand\dx[1][]{\diff{#1}{x}}

\usepackage{xfp}
\usepackage{pgfplots, pgfplotstable}
\usetikzlibrary{pgfplots.units}
\pgfplotsset{compat=1.15}

\usepackage{tikz}
\usepackage{float}

% Speed up the compile time for TIKZ
%\usepgfplotslibrary{external}
%\tikzexternalize

\newcommand{\placeholder}{[Insert Section Here]}


% Theorems and the such
\newtheorem{theorem}{Theorem}[section]
\newtheorem{corollary}{Corollary}[theorem]
\newtheorem{lemma}[theorem]{Lemma}
\newtheorem*{remark}{Remark}


% Exercise section
\newlist{exercise}{enumerate}{5}
\setlist[exercise]{label*=\arabic*.,ref=\arabic*,before={\subsection*{Exercises}}}
\let\ex\item

% Style to select only points from #1 to #2 (inclusive)
\pgfplotsset{select coords between index/.style 2 args={
    x filter/.code={
        \ifnum\coordindex<#1\def\pgfmathresult{}\fi
        \ifnum\coordindex>#2\def\pgfmathresult{}\fi
    }
}}

\DeclareMathOperator*{\argmax}{argmax}
\DeclareMathOperator*{\argmin}{argmin}
\DeclareMathOperator*{\avg}{average}

\title{The Art of Machine Learning}
\author{Zachary Ross}
\date{November 2019}

% Reformat the chapter and section titles
\setcounter{secnumdepth}{4}
\titleformat{\chapter}{\normalfont\sffamily\MakeUppercase}{\chaptertitlename\
\thechapter}{1em}{\bfseries\Large}
\titleformat{\section}{\normalfont\sffamily\bfseries\MakeUppercase}{
    \thesection}{1em}{}
\titleformat{\subsection}{\normalfont\sffamily\bfseries}{\thesubsection}{1em}{}
\titleformat{\subsubsection}{\normalfont\sffamily\bfseries}{\thesubsubsection}{1em}{}

% Method for seperating a subsection with letters
\newcounter{paragraphsection}
\renewcommand\theparagraphsection{\Alph{paragraphsection}}

\newenvironment{psec}[1]{\noindent\ignorespaces\refstepcounter{paragraphsection}
   \textbf{\theparagraphsection. #1.}}{}

% Shortcuts
\newcommand\recip[1]{\frac{1}{#1}}
